...
export function applyTimelineFilters(
  ...
  return events.filter(
    (event) =>
      ...
      matchesRoleQuickFilter(event, roleFilter) &&
      matchesFilterSet(event, filters)
  );
}

function buildFieldConfig(
  events: readonly ResponseItem[]
): FilterFieldsConfig<TimelineFilterValue> {
...
function matchesQuickFilter(event: ResponseItem, quickFilter: QuickFilter) {
  switch (quickFilter) {
    ...
    case 'tools':
      return ['FunctionCall', 'LocalShellCall', 'CustomToolCall', 'WebSearchCall'].includes(
        event.type
      );
    case 'files':
...
function matchesFilter(event: ResponseItem, filter: Filter<TimelineFilterValue>) {
  ...
  switch (filter.field) {
    ...
    case 'role':
      return compareValue(
        event.type === 'Message' ? event.role : undefined,
        values,
        filter.operator
      );
    default:
...
function compareValue(rawValue: string | undefined, allowed: string[], operator: string) {
