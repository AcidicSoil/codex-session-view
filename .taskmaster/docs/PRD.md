...
Current Codex Session Viewer is a powerful passive analysis tool but does not actively guide users during or after a session:
...
* Tooling authors building AGENTS-compliant TanStack apps and wanting feedback from real usage.
...
* ≥ 50% of post-session summaries and commit messages are generated via the tool.
...
* **Behavior**
  ...
  * Provide copy-to-clipboard and optional markdown download.
...
* **`components/viewer/ChatDock.tsx`** (wrapper/bridge)
  Depends on: [`components/chatbot/ChatDockPanel`].
...
**Exit Criteria**
Given a session + AGENTS rules, backend utilities can produce misalignments, summaries, and commit messages via direct calls.
...
1. **Inspect and remediate a misaligned session (Session Coach)**
...
* Use shadcn/ui and existing layout patterns.
...
* **Storage**
  ...
  * Existing filesystem/local session snapshots.
...
* **Use Vercel AI SDK for LLM calls**
  ...
  * Rationale: consistent streaming semantics and provider abstraction.
...
* Token-limit: over-long sessions trigger context trimming rather than crashes.
...
1. **AI provider outages/pricing changes**
   ...
   * Fallback: degrade to non-AI viewer only.
...
3. **Over-scoping General Chat**
   ...
   * Fallback: keep General Chat disabled (MODE_NOT_ENABLED) until ready.
...
* Codex Session Viewer README for base app architecture and features.
* RPG Method PRD template.
